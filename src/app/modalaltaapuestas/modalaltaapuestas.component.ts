...
@Component({
  selector: 'app-modalaltaapuestas',
  templateUrl: './modalaltaapuestas.component.html',
  styleUrls: ['../modalalta/modalalta.component.css']
})
export class ModalaltaapuestasComponent implements OnInit {
	private _url:string=localStorage.getItem("host")+"ecopocket/apuestas/insertar.php";
	form: FormGroup;
	deporte:any;
	cantidad:any;
	fechaActual:any;
  ...
  detalles:any;
	mensaje:any;
  constructor(fb: FormBuilder,private snackBar: MatSnackBar,private http:HttpClient,public dialogRef: MatDialogRef<ModalaltaapuestasComponent>,
        @Inject(MAT_DIALOG_DATA) lol) {
			this.form = fb.group({
					deporte: [this.deporte, Validators.required],
	                cantidad: [this.cantidad, Validators.required],
	                fecha: [this.fecha, Validators.required],
	                mercado: [this.mercado, Validators.required],
	                casa: [this.casa, Validators.required],
