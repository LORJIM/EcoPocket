...
export class OperacionescriptoComponent implements OnInit {
  private _url:string=localStorage.getItem("host")+"ecopocket/cripto/seleccionar.php";
  private _url2:string=localStorage.getItem("host")+"ecopocket/cripto/resolverOperacion.php";
  mensaje:any;
  arrayProfit:any;
  dataSource:any;
...
ngForTerminado(){ //ya mapeado el dataSource
    //Y AQUI APLICAMOS LOS ESTILOS PARA MOSTRAR FECHAS Y PROFITS EN LOS QUE CORRESPONDA:
          ...
          var element=$(".profit, .valorventa, .detalles, .dividendos"); //el elemento del html
          for(var i=0;i<element.length;i++){
            if(element[i].innerHTML==""){
...
onResolverOperacion(event: any,estado: string){
	...
	if(estado!='Neutro'){ //si es ganancia o perdida, es necesario que se abra una modal para que introduzcan el valor de venta
		const dialogRef = this.dialog.open(ModalresolvercriptoComponent, { //abrimos la ventana de confirmacion
			data:{ //este data sera el JSON que contenga la ID
		        ID:ID, //le pasamos la id en el matdialogdata
		        estado:estado //le pasamos el estado que depende del boton que hayamos hecho click
			},
      		width: '500px',
			height: '450px',
    	});
	...
	}else{ //si es neutro, directamente llamamos al php con valor de venta y dividendos null
		var datos={ID: ID, estado:estado, valorVenta: null, dividendos: null};
		//peticion php
...
onAlta(){
    const dialogRef = this.dialog.open(ModalaltacriptoComponent, {
      		width: '1000px',
			height: '400px',
    	});
...
onBaja(event: any){
    ...
    const dialogRef = this.dialog.open(ModalbajaComponent, { //abrimos la ventana de confirmacion
			data:{ //este data sera el JSON que contenga la ID
        ID:ID, //le pasamos la id en el matdialogdata
        tipo:'CR' //indicador de que venimos de Criptomonedas
			},
      		width: '800px',
			height: '200px',
    	});
