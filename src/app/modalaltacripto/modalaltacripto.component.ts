...
@Component({
  selector: 'app-modalaltacripto',
  templateUrl: './modalaltacripto.component.html',
  styleUrls: ['../modalalta/modalalta.component.css']
})
export class ModalaltacriptoComponent implements OnInit {
	private _url:string=localStorage.getItem("host")+"ecopocket/cripto/insertar.php";
	form: FormGroup;
	criptomoneda:any;
	importe:any;
	fechaActual:any;
  ...
  detalles:any;
	mensaje:any;
  constructor(fb: FormBuilder,private snackBar: MatSnackBar,private http:HttpClient,public dialogRef: MatDialogRef<ModalaltacriptoComponent>,
        @Inject(MAT_DIALOG_DATA) lol) {
			this.form = fb.group({
				criptomoneda: [this.criptomoneda, Validators.required],
