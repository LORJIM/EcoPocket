...
export class OperacionesforexComponent implements OnInit {
  private _url:string=localStorage.getItem("host")+"ecopocket/forex/seleccionar.php";
  private _url2:string=localStorage.getItem("host")+"ecopocket/forex/resolverOperacion.php";
  mensaje:any;
  arrayProfit:any;
  dataSource:any;
...
ngForTerminado(){ //ya mapeado el dataSource
    //Y AQUI APLICAMOS LOS ESTILOS PARA MOSTRAR FECHAS Y PROFITS EN LOS QUE CORRESPONDA:
          ...
          var element=$(".profit, .valorventa, .detalles"); //el elemento del html
          for(var i=0;i<element.length;i++){
            if(element[i].innerHTML==""){
...
consulta(){
  ...
  onResolverOperacion(event: any,estado: string){
	...
	if(estado!='Neutro'){ //si es ganancia o perdida, es necesario que se abra una modal para que introduzcan el valor de venta
		const dialogRef = this.dialog.open(ModalresolverforexComponent, { //abrimos la ventana de confirmacion
	...
	}else{ //si es neutro, directamente llamamos al php con valor de venta null
		var datos={ID: ID, estado:estado, valorVenta: null};
		//peticion php
...
onAlta(){
    const dialogRef = this.dialog.open(ModalaltaforexComponent, {
      		width: '1000px',
			height: '400px',
    	});
...
onBaja(event: any){
    ...
    const dialogRef = this.dialog.open(ModalbajaComponent, { //abrimos la ventana de confirmacion
			data:{ //este data sera el JSON que contenga la ID
        ID:ID, //le pasamos la id en el matdialogdata
        tipo:'F' //indicador de que venimos de Forex
			},
      		width: '800px',
			height: '200px',
    	});
